---
title: "Hypatia"
subtitle: "Single-Cell RNA Isoform Analysis"
author: "Timothy Pan"
date: today
date-format: short
...
Check out our <a href="https://www.biorxiv.org/content/10.64898/2026.01.13.699341v2" target="_blank">*preprint*</a> for more details on Hypatia. If you found this tool useful in your research, please remember to cite us:
...
The following dependencies will be installed if they are not already present: checkmate, dplyr (>= 1.0.0), ggnewscale, ggplot2, Matrix, patchwork, purrr, S4Vectors, SingleCellExperiment, SummarizedExperiment, tibble, tidyr, and matrixTests.
...
ggplot2::ggplot(plotdata) +
  ...
  ggplot2::geom_text(data = plotdata |> dplyr::distinct(cell_type, x_coord, y_coord), 
                     ...
                     size = 12 / .pt) +
  ggplot2::coord_fixed() +
  ggplot2::xlim(min(gbm_colData$umap_1), max(gbm_colData$umap_1) + 1) +
  ggplot2::xlab("UMAP 1") +
  ggplot2::ylab("UMAP 2") +
...
Hypatia uses the <a href="https://www.bioconductor.org/packages/devel/bioc/vignettes/SingleCellExperiment/inst/doc/intro.html" target="_blank">*SingleCellExperiment*</a> object, which provides a standardized framework for storing and organizing single-cell data.
...
1. By default, `CreateSCE()` searches for a column called `"gene_id"` in `rowData` for associated genes. This can be changed with `active.gene.id`. <br>
...
An empty `active.transcript.id` indicates that the row names of the count matrix (e.g., Ensembl transcript IDs in the tutorial data) should be used. An empty `active.group.id` means that cell identities for grouping (e.g., `cell_type`) must be specified in downstream functions. The `active.group.id` can be set in advance to avoid repeatedly defining the grouping variable later on.
...
Isoforms can also be filtered based on the provided transcript-level *SQANTI* metrics:
...
Afterwards, `SubsetCells()` can filter cells using these or other `colData` (cell-level) variables. Cells with fewer than 50 detected isoforms are removed below.
...
::: {.callout-note icon="true" appearance="simple"}
Vectors of cell IDs or active transcript IDs can also be passed through the `cells` and `transcripts` arguments in their respective subset functions.
:::
...
All three `Run*()` functions accept `group.by` as one or more `colData` column names. If `group.by` is omitted, Hypatia uses `metadata(object)$active.group.id`; multiple columns are combined with `_` in output labels. Column names containing spaces are supported when passed as strings. The `p.adj` argument accepts exact method names from `stats::p.adjust.methods`.
...
- `prop.diff`: The difference in transcript proportions between groups (`group.1` - `group.2`).

`$stats` contains results from statistical tests:

- `group.1` & `group.2`: The two cell groups being compared.
- `gene`: The gene being tested.
- `max.prop.diff`: The largest absolute difference in transcript proportions between `group.1` and `group.2`.
...
- `padj`: Adjusted p-value, calculated separately for each group comparison (default: Benjamini-Hochberg, `p.adj = "BH"`).
...
Let's look at the transcript proportions of the gene *MEG3*, which contains top DIU events from several comparisons.
...
plot_groups <- c("GABA neuron", "Glut neuron", "Microglia", "Oligodendrocyte", "OPC",
                 "Tumor")
...
`RunDIV()` compares isoform diversity using the Wilcoxon test. Gene-level diversities across cell groups are estimated by bootstrap iterations following gene filtering. By default, the Tsallis entropy is calculated. Genes are also classified as monoform or polyform. Set a random seed to ensure that results are reproducible. Refer to the help page (`?RunDIV`) for more details on parameters. The usage is similar to Hypatia's other 'Run' functions:
...
`$data` contains the bootstrapped isoform diversity data used to generate the statistical output.

- `group.1` & `group.2`: The two cell groups being compared.
- `gene`: Gene for which the isoform diversity is computed and compared.
- `gene.pct.1`: Percentage of cells in `group.1` with expression of the gene (without bootstrapping).
- `gene.pct.2`: Percentage of cells in `group.2` with expression of the gene (without bootstrapping).
- `n.transcripts`: Number of transcripts associated with the gene.
- `div.1`: Bootstrapped isoform diversities of the gene for cells in `group.1`.
- `div.2`: Bootstrapped isoform diversities of the gene for cells in `group.2`.

`$stats` contains results from the statistical tests.

- `group.1` & `group.2`: The two cell groups being compared.
- `gene`: Gene for which the isoform diversity is computed and compared.
- `avgDiv.1`: Average of bootstrapped isoform diversity of the gene for cells in `group.1`.
- `avgDiv.2`: Average of bootstrapped isoform diversity of the gene for cells in `group.2`.
- `div.diff`: Difference in averaged diversity (`group.1` - `group.2`).
- `log2FC`: Log2 fold change in averaged diversity for `group.1` relative to `group.2`.
- `pval`: P-value from the Wilcoxon rank-sum test comparing isoform diversity across the two groups.
- `padj`: Adjusted p-value (default: Bonferroni, `p.adj = "bonferroni"`).
...
example_genes <- c("NAPEPLD", "NPEPPS", "GNG7", "ANKRD10", "PNISR")
...
p_dens <- PlotDiversity(
  gbm, 
  genes = unique(unlist(gbm_div$data$gene)),
  plot.type = "density",
  group.subset = plot_groups,
  nrow = 2
)
...
# Shannon isoform diversity of gene MT3 in OPC cells
mt3_shannon_opc <- GetDiversity(gbm, genes = "MT3", group.subset = "OPC", entropy.use = "Shannon")
```
...
After normalization, `RunDEI()` can be used to compare isoform expression across cell groups. Group comparisons can be specified as shown:
...
- `log2FC`: The log2 fold change in transcript expression for `group.1` relative to `group.2`.
- `pval`: P-value from the Wilcoxon rank-sum test.
- `padj`: Adjusted p-value, calculated separately for each group comparison (default: Bonferroni, `p.adj = "bonferroni"`).
...
- `pct`: Percentage of cells in `group` with expression of the transcript.
- `avgExpr`: Average expression of the transcript across all cells in `group`.

To retrieve the expression matrix without summary statistics, users can combine `assay()` with `SubsetCells()` and `SubsetTranscripts()`. The following example retrieves the normalized expression matrix for isoforms of the gene *MBP* in oligodendrocytes:
...
oligod_mbp <- SubsetTranscripts(oligod, subset = gene_name == "MBP")
