...
Alternative splicing, transcriptional start sites, and termination sites are essential regulatory mechanisms that produce RNA isoforms that diversify the coding capacity of the genome with significant implications in health and disease. Single-cell long-read RNA-sequencing is a novel technology enables the investigation of RNA isoforms at cell type resolution. 

Hypatia is a computational suite in R for the analysis of single-cell isoform expression data. There are 3 analysis modules offered in Hypatia: differential isoform usage (DIU), differential isoform diversity (DIV), and differential isoform usage (DEI). This vignette shows how to analyze data using Hypatia and provides an full tutorial data set that is pre-built into the package. 
...
** This vignette will be hosted on Github Pages and accessible via URL once the repository is made public.
...
The tutorial data is available (lazy-loaded) once the package is loaded into the environment:
...
The raw isoform expression matrix `gbm_countData` consists of 56,642 isoforms from 5,043 cells.

```{r expr-str}
#| eval: true
#| collapse: true
#| code-fold: show
#| code-summary: Raw count matrix
...
#| collapse: true
#| code-fold: show
#| code-summary: Cell-level metadata
...
#| collapse: true
#| code-fold: show
#| code-summary: Transcript-level metadata
...
Hypatia provides **3** modes of differential isoform analysis:

-   **Isoform Usage**: Identifies differential isoform usage (DIU) events, marked by significant differences in isoform ratios of genes.

-   **Isoform Diversity**: Computes isoform diversity, classifies genes as 'monoform' (low diversity) or 'polyform' (high diversity), and performs pairwise comparisons of classifications. Genes having diverging classifications across are considered DIVs.

-   **Isoform Expression**: Identifies differentially expressed isoforms (DEIs) by comparing the mean normalized expression of isoforms between cells.
...
`RunDIU()` returns a list with two elements:
...
`RunDIV()` returns a list with two elements:
