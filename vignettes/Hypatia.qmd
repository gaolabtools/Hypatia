...
- `boot.iter`: Bootstrap iteration number.
- `group.1` & `group.2`: The two cell groups being compared.
- `gene`: Gene list for which the isoform diversity is computed.
- `div.1`: Bootstrapped isoform diversity of the gene for cells in `group.1`, ordered by `gene`.
- `div.2`: Bootstrapped isoform diversity of the gene for cells in `group.2`, ordered by `gene`.
...
- `delta.div`: Difference in averaged diversity (`group.1` - `group.2`).
- `log2FC`: Log2 fold-change of averaged diversity (`group.1` - `group.2`).
- `pval`: P-value from the Wilcoxon rank sum test comparing isoform diversity across the two groups. 
- `padj`: Adjusted p-value (Bonferroni correction).
...
mt3_div_opc <- GetDiversity(gbm, genes = "MT3", group.subset = "OPC", entropy.use = "Shannon")
```
